...
public class DamageNumberSpawner : MonoBehaviour
{
    ...
    private void SpawnPlayerDamageNumber(int damage, bool hasBlocked)
    {
        ...
        var damageNumber = Instantiate(damageNumberPrefab, screenPoint, transform.rotation, transform).GetComponent<DamangeNumber>();
        damageNumber.WorldSpawnPont = Player.Instance.transform.position;
    ...
    private void SpawnEnemyDamageNumber(int damage)
    {
        ...
        var damageNumber = Instantiate(damageNumberPrefab, screenPoint, transform.rotation, transform).GetComponent<DamangeNumber>();
        damageNumber.WorldSpawnPont = FindObjectOfType<EnemyCombatHandler>().transform.position;
        damageNumber.Damage = damage;
    }
}